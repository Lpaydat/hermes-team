...
- **Child ticket**: `bd create --title="<ticket name>" --parent=<map-id> --labels=wayfinder:<type> --no-inherit-labels --description="## Question ..."` with `<type>` one of `research`/`prototype`/`grilling`/`task`. `--no-inherit-labels` keeps the child from inheriting `wayfinder:map`. List children with `bd children <map-id>`.
