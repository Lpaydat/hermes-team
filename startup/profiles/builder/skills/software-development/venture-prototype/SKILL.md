...
- **Subagents completing the parent card.** When you use `delegate_task` for research or verification, leaf subagents inherit the kanban toolset and can call `kanban_complete` on your parent task. This causes the card to complete before the work is done. **ALWAYS pass `enabled_toolsets=["file","terminal"]` to delegate_task** to restrict leaf subagents to only the tools they need. NEVER give a research/verifier subagent the kanban toolset — it has no business managing task lifecycle. **Confirmed TWICE**: (1) V11 LeadPilot A/B loop test (2026-08-01): a README verifier subagent called `kanban_complete` on the parent task while the orchestrator was still working. (2) S07 F2_threat_observed (2026-08-01): Phase 2 verifier subagent called `kanban_complete` on the parent task, prematurely marking it done before the orchestrator posted the review handoff or updated the portfolio. In both cases the work was complete so no harm, but it bypassed the structured handoff. **The fix is simple: add `enabled_toolsets=["file","terminal"]` to EVERY delegate_task call for verifiers.**
