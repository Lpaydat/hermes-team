...
1. **Localized bug (default)** → ship a *proven* minimal fix + a regression test + a post-mortem (RCA), then create a verifier card (`assignee: verifier`) on the same board with the bug branch reference, what was fixed, and how to verify the merge. The verifier reviews, merges the fix branch to main. The workflow engine automatically creates a QA re-test card when it detects the new commit on master. The bug bead closes via bead-sync when the merge lands. **Already-fixed case:** if the bug doesn't reproduce on master because a prior fix addressed it, close the bead — the engine's QA trigger will verify the running artifact.
