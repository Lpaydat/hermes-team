...
**Key fields:** `session_id` for resumption, `num_turns` for agentic loop count, `total_cost_usd` for spend tracking, `subtype` for success/error detection (`success`, `error_max_turns`, `error_during_execution`).
...
| ~~`--max-budget-usd`~~ | DOES NOT EXIST in claude 2.x (verified 2.0.5: "unknown option") — budget via `timeout` wrapper + `--max-turns` + post-hoc `total_cost_usd` check |
...
2. **Cost caps**: no budget flag exists (verified claude 2.0.5) — wrap with `timeout`, cap `--max-turns`, and assert `total_cost_usd` from the JSON envelope after the run.
...
3. **No `--max-budget-usd` flag** — the CLI rejects it as an unknown option (verified 2.0.5). Budget = turn cap + wall-clock timeout + post-hoc cost assertion.
