...
# Checks if the last pipeline cycle was within 3 days.
...
# These are checked via the agent's kanban tools, not here —
# the agent itself decides if urgent work exists.

NOW_EPOCH=$(date +%s)
COOLDOWN_DAYS=3
COOLDOWN_SECONDS=$((COOLDOWN_DAYS * 86400))
MARKER="$HOME/vault/.last-pipeline"
...
if [ -f "$MARKER" ]; then
    ...
    if [ "$ELAPSED" -lt "$COOLDOWN_SECONDS" ]; then
        # Cooldown active — do the urgent checks HERE (zero tokens) instead of
        # waking the agent to do them. Wake only if board activity exists since
        # the last pipeline run; fail-open (wake) if the kanban CLI errors.
        URGENT=""
...
")
        ...
        if [ -n "$URGENT" ]; then
            ...
            echo "URGENT:$URGENT"
            echo "ACTION:RUN — cooldown active but board activity since last pipeline. Run the relevant phase (build review or feedback processing) for the items above."
        else
            echo "STATUS:RECENT"
