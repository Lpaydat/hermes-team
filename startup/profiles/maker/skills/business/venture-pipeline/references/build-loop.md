...
You are the FOUNDER/ANSWERER, not the griller — PO is the interviewer. Do NOT send unsolicited messages to
PO. The venture enters the autonomous loop through a kanban intake card, not an kanban comment.
...
**Grill card topic:** `venture-pitch/<slug>` — put this in the card body so PO knows
...
of PO's kanban comments (topic `venture-pitch/<slug>`) makes the broker RESUME your prior
...
3. **Reply** via the `kanban` tool: `action=reply`, `reply_to=<id>` (the id is in the ask).
...
iteration N, awaiting gate review, approved) in `~/vault/portfolio.md`. When the
