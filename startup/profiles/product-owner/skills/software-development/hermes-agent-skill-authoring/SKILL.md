...
22. **Eating your own dog food: use the dev workflow to build dev workflow features (Jul 2026).** The intercom feature was built entirely through the dev workflow pipeline: PO wrote PRD + beads, workflow engine dispatched, tech-lead delegated, developer built, verifier reviewed — 4 beads, 3 fix chains, 2 auto-resolved escalations, 47 tests, zero human intervention. The temptation to jump straight to coding (bypassing the workflow) is strong, especially when the feature IS the workflow. **Rule**: when building a new feature, resist the urge to code it manually. Run `dev-planning` → `to-spec` → `to-tickets` → let the pipeline build it. The system building its own next feature is the ultimate test of the workflow.

23. **External skill packs must be read-only to prevent self-evolution corruption (Jul 2026).** Installed skill packs like Matt Pocock and Ponytail live in `~/.hermes-teams/shared-skills/<pack>/` with `chmod 555` (dirs) / `chmod 444` (files). All team profiles symlink directly: `profiles/<p>/skills/mattpocock → ../../../../shared-skills/mattpocock`. This read-only layer is what prevents Hermes self-evolution from patching third-party skills it doesn't own. The npx installer (`npx skills@latest add mattpocock/skills`) puts skills in `~/.agents/skills/` with writable perms (755/644) — fine for the default profile, but **unsafe for team profiles** that have self-evolution enabled. **Update workflow when a new version arrives**: (1) unlock old dir (`chmod -R u+w`), (2) delete old `shared-skills/<pack>/` + any `-hub/` dir, (3) copy fresh from `~/.agents/skills/` into `shared-skills/<pack>/` (no version suffix — same name), (4) lock down: `find ... -type d -exec chmod 555 {} \; && find ... -type f -exec chmod 444 {} \;`, (5) verify write protection with `touch` test, (6) repoint all profile symlinks (they already point to `../../../../shared-skills/<pack>` so if the name is unchanged, only the old `-hub/` indirection needs removing). Never repoint team profile symlinks directly to `~/.agents/skills/` — that bypasses the read-only protection layer.

24. **When upstream renames skills, sweep ALL reference types (Jul 2026).** The mattpocock skills upstream renamed `to-prd`→`to-spec` and `to-issues`→`to-tickets` (kept old names for backward compat). After updating the skill pack, stale references remained across the system. The sweep must cover: (a) **config.yaml `skills:` lists** in all 9+ profiles, (b) **prose in custom skills** (`\`to-prd\`` → `\`to-spec\``), (c) **`.gitignore`** paths (e.g. `mattpocock-hub` → `mattpocock`), (d) **shared-skills README.md** install instructions, (e) **po-role-boundaries.md and SOUL.md** role docs, (f) **`.usage.json`** keys (cosmetic), (g) **delete `.bak` files** (don't update them — they're stale snapshots). **Skip** historical records: `sessions/`, `cron/output/`, battle-test results (`RESULTS-*.md`), ADRs — those document what was true at the time. Run subagents in parallel to scan `~/.hermes-teams/`, `~/workspace/`, and cron jobs — the scan itself is fast with `rg`. Verify with a final grep excluding the skill pack's own directory (it legitimately contains old names for backward compat). When old skill names still appear as slash commands after updating configs, check whether the old skill _directories_ still physically exist in the shared-skills hub (npx installs both old+new for backward compat) — physically remove the old dirs, not just the config references.

25. **Missing live-testing AC: the gap that shipped 4 bugs (Jul 2026).** The intercom system shipped with 4 integration bugs — spawner read stdout instead of stderr (session_id always lost), string content silently dropped, direction-sensitive conversation keys (A→B ≠ B→A), bidirectional spawn session collision. All 4 passed unit tests (47+) and adversarial code review. None were caught until a human said "test it to extreme" and someone actually ran the assembled system with real Hermes sessions. **Root cause**: acceptance criteria were written as unit-test-level assertions ("spawner captures session_id", "all tests pass") — none required running the actual artifact end-to-end. **Rule**: when planning any feature that produces a runnable artifact (CLI, API, broker, daemon, plugin, library), the ACs MUST include at least one live-integration criterion: "build it, run it, use it like a real user would, verify actual behavior." Unit tests prove the pieces; they don't prove the assembled thing works. The QA profile (live-testing gate, post-merge) exists to enforce this empirically — but the AC must _ask_ for it during planning.
