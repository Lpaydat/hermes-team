...
- **Expecting parallel execution from same-profile fan-out** — `max_in_progress_per_profile` is set in each profile's own `config.yaml`, and the dispatcher reads the **lock-holding gateway's profile config** (not a global config), so it caps each profile to N concurrent tasks; child cards with the same assignee run up to N at a time. Raise the cap (in every profile config) or use different profiles for more parallelism.
...
- **Kanban caps live in each profile's `startup/profiles/<profile>/config.yaml`, not in a global config.** The dispatcher reads the **lock-holding gateway's OWN profile config** (whichever gateway holds `startup/kanban/.dispatcher.lock`), NOT the global `startup/config.yaml` and NOT `~/.hermes/config.yaml`. Because the lock-holder is non-deterministic, **all profile configs must agree** on kanban caps — edit every profile's `config.yaml`, not just one, for a cap change to take effect regardless of which gateway dispatches; then restart the lock-holding gateway.
