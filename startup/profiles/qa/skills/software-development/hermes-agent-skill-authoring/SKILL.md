...
4. **Validate frontmatter locally**:
   ```python
   ...
   ```
5. **Quality review — MANDATORY, before commit.** YAML validation checks structure; this checks craft. Re-read the SKILL.md and every reference file you wrote, sentence by sentence, against the Writing Quality Principles above and the checklist in `references/quality-review-checklist.md`. Focus on the three failure modes that hit hardest:
   - **Negation**: scan for "do not", "don't", "never", "no X" used as steering. Rewrite each as the positive target behavior. A prohibition like "install profile-scoped, never globally" is acceptable only as a hard guardrail paired with its positive ("install profile-scoped").
   - **Duplication with SOUL/other skills**: if the SOUL.md or another loaded skill already establishes an identity or rule every turn, restating it in the skill pays tokens to say nothing. Cut it.
   - **No-op prose**: "Push boundaries. This is where real bugs live." The model knows this. Delete it.
   **Do NOT skip this step or substitute an automated grep for a read-through** — grepping for patterns finds what you already know to look for; reading catches what you don't. See Pitfall #10.
...
10. **Substituting an automated grep for a quality read-through.** After writing a skill, grepping for negation patterns or counting lines feels like "reviewing" but finds only what you already know to look for. The failure modes that matter — duplication with another loaded skill, no-op prose that reads like good advice but changes nothing, edge-case tables that re-list universal categories — require actually reading every file you wrote, sentence by sentence. The grep is a supplement, not a substitute. A review that didn't read the files didn't happen.
...
- [ ] **Quality review completed** (Workflow Step 5) — every file read sentence by sentence against `references/quality-review-checklist.md`, not grepped
