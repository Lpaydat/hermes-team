...
15. **Redirecting stdout to /dev/full on the `podman run` command line.** `podman run … img txtodo list t.txt >/dev/full` breaks *podman's own attach layer* (rc=126, "attaching to container …: write /dev/stdout: no space left on device") — you tested podman, not the artifact. The redirection must happen INSIDE the container: `podman run … img sh -c 'txtodo list t.txt >/dev/full; echo rc=$?'`. Same rule for any fd-redirection probe under a container runtime.

16. **Testing an old commit from a worktree with a venv that has the package installed.** `python -m txtodo.cli …` resolves the module from the venv's site-packages (the NEW code), not the worktree — a "pre-fix baseline" silently runs the fix and invalidates classification. Always set `PYTHONPATH=<worktree>/src` (or pip-install from the worktree) and assert resolution first: `python -c "import txtodo; print(txtodo.__file__)"`. Symptom: traceback file paths pointing at site-packages instead of the worktree.

17. **Signal-guard tests that close the pipe after sending the signal mask the full-pipe hang class.** A landed test doing `proc.send_signal(SIGINT); proc.stdout.close(); proc.wait()` converts any blocked `write(1,…)` into EPIPE — it can never detect that a single SIGINT leaves the process hung when stdout is a full pipe with a paused reader (interpreter-shutdown flush re-blocks). To probe the real user scenario, keep the pipe open and `communicate(timeout=…)`; confirm the mechanism via `/proc/<pid>/syscall` (write nr 1, state S) and a drain-the-pipe probe (hang + drain → clean rc proves flush-re-block). Always run a **norm control** through the identical harness — bare `python -c 'print(big)'` and `seq 1 3000000` — before filing: if the controls also hang, it's platform behavior, not an artifact defect.

18. **Double-SIGINT probes: back-to-back signals COALESCE and the hang persists.** A second SIGINT sent immediately after the first is delivered while CPython still has the handler armed (or before unwinding restores SIG_DFL), so the process stays blocked. Space the second signal ≥300ms after the first (observed: spaced → rc -2 kill; back-to-back → still hung). Any test asserting "two SIGINTs always kill" must include that spacing, or it will flake by timing. Also: a test whose docstring documents hang-until-second-SIGINT while its body asserts prompt rc-130 exit is self-contradicting — such a file landed red on master because only the docstring matched reality.

19. **Re-test cards can misdescribe what landed — verify the cited range BEFORE testing.** Cards are often generated from verifier-completion summaries, not merge events, so "What landed: X" can be empirically false: the cited commit range may hold a different merge entirely, or name the range's BASE commit's feature instead of its content (base-vs-content flavor). First action on any re-test card: `git fetch origin`, then `git log --oneline <base>..<merge>` and `git diff --name-only <base>..<merge>` — test what the range ACTUALLY contains, per the card's own instruction to verify landed content over summary claims. File premise mismatches as a note on the project's tracking bead (don't spawn a new one per instance). Re-check the range for drift before declaring FAIL — merges can land mid-run.
