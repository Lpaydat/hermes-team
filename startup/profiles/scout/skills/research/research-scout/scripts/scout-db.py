#!/usr/bin/env python3
"""
Scout DB — SQLite helper for dedup, source analytics, and topic tracking.

This is the canonical copy bundled with the research-scout skill.
A profile-level copy also lives at ~/.hermes/profiles/research/scripts/scout-db.py
(kept in sync). The profile-level copy is what the cron job and daily scout invoke.
...
Usage:
  ...
  python3 scout-db.py register <url> <title> <depth> <wiki_note> [source_tier]
  python3 scout-db.py update <url_or_id>                   # Bump update_count + last_updated
  python3 scout-db.py set-note <url_or_id> <wiki_note>    # Set wiki_note for a processed item (deep-research uses this)
  python3 scout-db.py source-add <name> <url> <tier>
  python3 scout-db.py source-touch <name> [items] [deep_dives]
  python3 scout-db.py source-prune <name>                  # Mark source as pruned
  python3 scout-db.py source-stats                         # Source quality rankings
...
The DB lives at ~/vault/meta/scout.db. All writes are atomic (WAL mode).
"""
...
def extract_arxiv_id(identifier):
    ...
    for prefix in ["arxiv.org/abs/", "arxiv.org/pdf/"]:
        if prefix in identifier:
            return identifier.split(prefix)[-1].replace(".pdf", "").split("v")[0]
    return None
...
def update(identifier):
    ...
    now = datetime.now().isoformat(timespec="seconds")
    conn = get_db()
    result = conn.execute("UPDATE processed SET update_count = update_count + 1, last_updated = ? WHERE url_hash = ?", (now, h))
    conn.commit()
    if result.rowcount > 0:
        print(f"OK: Updated (hash:{h})")
    else:
        print(f"ERROR: Not found (hash:{h}). Use 'register' first.")
    conn.close()

def set_note(identifier, wiki_note):
    """Set the wiki_note field for a processed item (used by deep-research after creating a note)."""
    h, normalized = make_hash(identifier)
    now = datetime.now().isoformat(timespec="seconds")
    conn = get_db()
    result = conn.execute("UPDATE processed SET wiki_note = ?, last_updated = ? WHERE url_hash = ?", (wiki_note, now, h))
    conn.commit()
    if result.rowcount > 0:
        print(f"OK: Set wiki_note='{wiki_note}' for hash:{h}")
    else:
        print(f"ERROR: Not found (hash:{h}). Use 'register' first.")
    conn.close()
...
def source_add(name, url, tier):
    ...
    conn = get_db()
    try:
        conn.execute("INSERT INTO sources (name, url, tier, first_seen, last_seen, quality_score) VALUES (?, ?, ?, ?, ?, 0)", (name, url, tier, now, now))
        conn.commit()
...
def source_touch(name, items=1, deep_dives=0):
    ...
    if row:
        ...
        conn.execute("UPDATE sources SET items_produced = ?, deep_dives_produced = ?, last_seen = ?, quality_score = ? WHERE name = ?", (new_items, new_deep, now, score, name))
        conn.commit()
...
def source_stats():
    conn = get_db()
    rows = conn.execute("SELECT name, tier, items_produced, deep_dives_produced, quality_score, pruned, last_seen FROM sources WHERE pruned = 0 ORDER BY quality_score DESC, deep_dives_produced DESC").fetchall()
    conn.close()
    if not rows:
...
def topic_touch(topic, note=None):
    ...
    if row:
        ...
        conn.execute("UPDATE topics SET mention_count = ?, last_seen = ?, related_notes = ? WHERE topic = ?", (new_count, now, related, topic))
        conn.commit()
        print(f"OK: Topic '{topic}' now {new_count} mentions")
    else:
        conn.execute("INSERT INTO topics (topic, first_seen, last_seen, mention_count, related_notes) VALUES (?, ?, ?, 1, ?)", (topic, now, now, note))
        conn.commit()
...
def topic_trending(days=7):
    ...
    conn = get_db()
    rows = conn.execute("SELECT topic, mention_count, last_seen, related_notes FROM topics WHERE last_seen >= ? AND topic != '' ORDER BY mention_count DESC, last_seen DESC LIMIT 20", (cutoff,)).fetchall()
    conn.close()
    if not rows:
...
def stale_notes(days=30):
    cutoff = (datetime.now() - timedelta(days=days)).isoformat(timespec="seconds")
    conn = get_db()
    rows = conn.execute("SELECT title, wiki_note, last_updated, depth_tier FROM processed WHERE depth_tier IN ('deep-dive', 'notable') AND last_updated < ? ORDER BY last_updated ASC LIMIT 20", (cutoff,)).fetchall()
    conn.close()
    if not rows:
...
def main():
    ...
    if cmd == "init": init()
        ...
        if len(sys.argv) < 3: print("ERROR: Missing identifier"); sys.exit(1)
        dedup_check(sys.argv[2])
    elif cmd == "register":
        if len(sys.argv) < 6: print("ERROR: Usage: register <url> <title> <depth> <wiki_note> [source_tier]"); sys.exit(1)
        source_tier = sys.argv[6] if len(sys.argv) > 6 else None
        register(sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], source_tier)
    elif cmd == "update":
        if len(sys.argv) < 3: print("ERROR: Missing identifier"); sys.exit(1)
        update(sys.argv[2])
    elif cmd == "set-note":
        if len(sys.argv) < 4: print("ERROR: Usage: set-note <url_or_id> <wiki_note>"); sys.exit(1)
        set_note(sys.argv[2], sys.argv[3])
    elif cmd == "source-add":
        if len(sys.argv) < 5: print("ERROR: Usage: source-add <name> <url> <tier>"); sys.exit(1)
        source_add(sys.argv[2], sys.argv[3], sys.argv[4])
    elif cmd == "source-touch":
        if len(sys.argv) < 3: print("ERROR: Usage: source-touch <name> [items] [deep_dives]"); sys.exit(1)
        ...
        if len(sys.argv) < 3: print("ERROR: Usage: source-prune <name>"); sys.exit(1)
        source_prune(sys.argv[2])
    elif cmd == "source-stats": source_stats()
    elif cmd == "topic-touch":
        if len(sys.argv) < 3: print("ERROR: Usage: topic-touch <topic> [note]"); sys.exit(1)
