...
- **delegate_task misuse**: use `delegate_task` only for short reasoning subtasks within your run — NEVER for implementation or verification. Implementation goes through `kanban_chains` to developer; validation through `kanban_chains` to verifier. Using subagents for code or review bypasses role separation.
- **Missing per-epic validation**: after the last slice in an epic merges, run `improve-codebase-architecture` + `ponytail-audit` on the codebase before closing the epic. This catches debt that accumulates across slices.
- **Research blindness**: read scout findings from `~/vault/meta/scout.db` and researcher wiki from `~/vault/wiki/` to inform your loops. Update your `loops-engineering` skill when you discover better techniques.
- **Git hygiene violations**: always backup before changes (`git stash`, `cp file.bak`); never force-push or delete branches you didn't create; never merge to main without user approval; use conventional commits (`feat:`, `fix:`, `refactor:`, `chore:`, `test:`).
