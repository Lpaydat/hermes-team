...
How the five-phase loop runs when the generator and evaluator are separate Hermes profiles (`developer`, `reviewer`) coordinating through cards.
...
| Evaluator | `reviewer` profile | **Trace-blind by default** (independence); reads diff + files + executed tests + completion report; opens transcript only on tamper suspicion |
...
bead ready
  ...
  → verifier works card (adversarial-review skill): EXECUTE tests/evals/build →
      evidence-gated findings → verify-findings pass
      → PASS → bd merge-slot (--holder reviewer --wait): rebase → re-run suite on rebased
               candidate → merge → release → complete card → completion boundary closes the bead
...
Retry cap: the iteration count is a COMMENT convention — the reviewer heads each findings comment `REVIEW-ITERATION: <N>` and stamps `Review-Iteration: <N+1>` into the fix card's body (kanban cards have no mutable metadata field, and `kanban_create` accepts no metadata — verified in platform source; `consecutive_failures` counts spawn crashes only). The review-card chain under the dev card is the fallback count. Watchdog audit tick (count iterations ≥3 without an escalation card; reconcile done dev cards against still-open beads): **TODO — not yet implemented in beads-watchdog.sh**; until it is, the reviewer's escalation discipline is the only enforcement.
...
3. Only the completion boundary writes back to beads (the reviewer's pass-merge-complete is that boundary).
...
7. **Product-owner amendment**: product-owner owns bead CONTENT — it files beads from discovery and is the only role that changes what a bead promises. Tech-lead owns the contract derived from a bead. Reviewer-found gaps split mechanically: *contract-vs-code* → tech-lead (re-contract); *contract-vs-intent* (the bead promises the wrong thing) → tech-lead route to verifier never re-contract anyone.

## Merge story: serialized merge-owner (no auto-merge queue)

`bd merge-slot` (shipped in bd 1.0.4 — Gastown's Refinery serialization primitive) + reviewer-as-merge-owner:

- Developers NEVER merge; every card ends on its branch.
- Reviewer, on pass: acquire slot → rebase onto main → **re-run evals + full suite on the rebased candidate** (the DoltHub rule: a queue that trusted reported green auto-merged failing tests) → merge → release slot. Rebase conflicts = review FAIL (conflict resolution is code-writing → fix card to developer).
- Skip bisection below ~4 concurrent developers; serialization is the strategy.

## When to route here vs harness-direct

Routing table + tiebreakers: see orchestration-models.md. Summary: harness-direct stays the DEFAULT; route kanban-native when the work is genuinely parallel (≥3 independent beads — matches the consultation's T2 threshold; with the per-profile cap at 1, parallelism won't materialize anyway, so the durability/audit criteria are what usually qualify a card), multi-module, session-spanning, or audit-required — and the contract already exists. Never dispatch an ambiguous card.
...
1. Median wall-clock per iterate round (kill criterion: >45 min sustained → retreat to harness-direct + reviewer-only, keep the trace ledger).
