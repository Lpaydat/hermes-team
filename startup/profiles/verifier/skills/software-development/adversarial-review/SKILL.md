...
**If the park did NOT verify** (`block_verified: false` / an error mentioning "Park did not take effect" / "Block did not take effect"): the swarm topology already exists and your links already exist. There is exactly ONE correct repair, in this order:
1. `kanban_show` YOUR card and read the **PARENTS** list (not children — the park links are PARENT links: worker → you). If the workers are listed as parents, go to 2.
2. Call `kanban_block` on YOUR card with `kind='dependency'` and reason `"dependency: waiting for <worker-ids>"`. ONE call. If it errors with "not in running/ready", your card already moved — re-read status and follow it.
3. If parents are missing, `kanban_link <worker_id> <your_card>` for each missing worker, then step 2.

**NEVER a plain block** (no `kind`) — it is sticky, never auto-promotes, and deadlocks the whole chain. **NEVER re-call `kanban_chains`** for a park failure — that creates a duplicate swarm. Park failures are repaired with link+block only.
...
Then the **ADR-conformance lens**: does the diff honor the venture's ADRs (`docs/adr/` per `docs/agents/adr-convention.md`) and the spec's architecture section (the contract's/card body's architecture or design section, when one is carried)? Architectural drift with zero functional bugs is still a severity-graded finding, and the finding MUST name the violated ADR by number (file:line + the quoted ADR Decision line it violates, like any contract violation). Complete with the conformance result in your structured metadata — violated (with the ADR ids) | clean (with the ADR count checked) | skipped — and when ADRs exist and no drift is found, your completion summary reports the conformance check ran clean ("ADR conformance: clean, N ADRs checked"). When the repo has no `docs/adr/`, the ADR half of this lens is a no-op — stamp `adr_conformance {"status":"skipped","reason":"no-docs-adr"}` (the metadata `reason` is exactly the token `no-docs-adr`, not free prose), note "no docs/adr/ — conformance lens skipped" for humans, and file nothing (no noise findings on ADR-less repos); the spec's architecture section is still checked.
...
**Pre-flight (before the kanban_complete call):** re-read the ENTIRE metadata JSON you are about to submit, key by key. Under completion pressure, ids get typo'd (`t_92e7e90` for `t_92e82990`), citation objects get malformed (`"artifact_type": "b: 5/5 stories..."`), and fields get fabricated to look complete (an invented `worker_session_id`). A mangled stamp poisons dashboards and parent-card injection silently — and a fabricated field is a honesty breach, not a typo. If a value is unknown, OMIT the key; if the damage is already landed, post an errata comment on the card naming every wrong key and the true values (observed live: run-27 verdict needed a 3-point errata while its dod_met/recommendation/gaps were all correct).
...
**ESCALATE**: call `kanban_chains` to create an escalation card assigned to `tech-lead` (or `product-owner` for contract-vs-INTENT gaps). This dependency-parks YOUR card (status=todo, kind=dependency) — it auto-promotes when the escalation card completes, so you wake up to stamp the final verdict. Do NOT use `kanban_block` — a sticky block never auto-promotes and deadlocks the chain. The escalation card body MUST carry the verdict fields (`{verdict: ESCALATE, findings_count, iteration, acs_verified, ...}`) plus your session id. See [verdict-routing.md](references/verdict-routing.md).
...
- **Phantom park (verified block, missing links)**: `kanban_chains` may record a `dependency_wait` event (park looks verified) yet create NO worker→you parent links — your only parent stays the completed dev card, so the dispatcher promotes you ~1 tick later while the swarm is still running. Signature: you are re-dispatched and probe cards are incomplete (todo/running, no completions in context). Repair is the same link+block procedure above — do NOT treat re-dispatch as synthesis mode until you have confirmed the probe cards COMPLETED; check their status first (one `kanban_show` each or a read-only DB read), link every incomplete probe as your parent, then `kanban_block(kind='dependency')`.
- **Shared-repo gate race with your own swarm**: if you re-dispatch mid-swarm, your delta/fresh-eyes workers are likely running `npm test`/`npx playwright test` in the SHARED repo right now. Do NOT run your own gates, mutation checks, or any repo-mutating command until the swarm completes — a concurrent checkout/clean race poisons both your evidence and theirs. Park first; Stage 1/3 execution resumes on the next promotion.
...
- **Mutation-apply verification by grep can false-NOOP**: confirming a sed mutation "applied" by grepping for the original code string can match the SAME string in a docstring/comment (e.g. `depth <= max_depth` appears in both the comprehension and its docstring) → you wrongly report "SED-NOOP" and exit before restoring, leaving the clone mutated mid-loop. Verify mutations by grepping the mutated FORM (or diffing the region), never by absence of the original string; and structure mutation scripts so a detection miss NEVER skips the `git checkout --` restore (restore on every exit path).
...
- **Mutation-restore pyc staleness**: restoring a same-size mutated file via `cp` can land within the same mtime second as the mutated version, so Python's bytecode cache validator (size + seconds-granularity mtime) keeps executing the STALE MUTATED `.pyc` over pristine source — the post-restore "green" run fails on code you know is correct, and a NOT-CAUGHT result can also be faked in reverse. Mitigate: `find . -name __pycache__ -type d -prune -exec rm -rf {} +` before EVERY pytest invocation in the mutation loop, export `PYTHONDONTWRITEBYTECODE=1`, and sha256-verify the restored file against the git object before citing any post-restore green. (Observed live: verifier-side false FAIL on rc assertion with correct source on disk; rc=0 came from stale M3-mutated bytecode.)
...
- **Mutation-clone litter**: a mutation harness that leaves its /tmp clone behind fails the post-turn workspace-verify wrapper (and leaves node_modules symlinks dangling). End every mutation script with: unlink the clone's node_modules symlink FIRST (os.unlink if islink — never rmtree through it into the real node_modules), then shutil.rmtree(CLONE); assert the clone is gone before exiting. Structure wrappers to re-check `glob('/tmp/hermes-verify-<prefix>*')` is empty. (Observed live: first wrapper run FAIL on leftover clone; harness patched to self-clean, second run ALL-HOLD.)
- **Shared-repo wipe by concurrent waves**: when several verifiers share ONE repo (integration reviews on master), a concurrent wave's commit + `git clean` / `checkout -- .` deletes YOUR untracked test files mid-review (observed live: a behavior-test file vanished between two test runs). Keep the canonical copy of every test file in YOUR task workspace (`$HERMES_KANBAN_WORKSPACE/`) and re-`cp` it into the repo on each execution pass; run mutation checks on a throwaway `git clone` under `/tmp`, never on the shared checkout. Re-verify `git rev-parse HEAD` before citing evidence — concurrent waves move master under you. Landing your committed matrix on the shared repo when its index is dirty with another wave's staged files: `git am` refuses ("Dirty index"), so `cp` the canonical file in, `git add --intent-to-add <file>` (untracked pathspecs fail `commit --only` without it), then `git commit --only <file> -m ...` — commits just your path, leaves their staged state intact. Also pin the parent SHA you verified at and diff `src/` between it and the landing HEAD to keep mutation-check evidence valid.
